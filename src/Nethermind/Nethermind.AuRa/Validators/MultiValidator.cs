...
namespace Nethermind.AuRa.Validators
{
    public class MultiValidator : IAuRaValidatorProcessor
    {
        ...
        public MultiValidator(AuRaParameters.Validator validator, AuRaParameters parameters, IAuRaAdditionalBlockProcessorFactory validatorFactory, IBlockTree blockTree, IValidatorStore validatorStore, ILogManager logManager)
        {
        ...
        {
            bool ValidatorWasAlreadyFinalized(KeyValuePair<long, AuRaParameters.Validator> validator) => _blockFinalizationManager.LastFinalizedBlockLevel >= validator.Key;
            
            if (TryGetLastValidator(blockNumber, out validatorInfo))
            {
                if (forceChange || CanChangeValidatorImmediately(validatorInfo.Value) ||  ValidatorWasAlreadyFinalized(validatorInfo)) 
                {
        ...
        private void SetCurrentValidator(long finalizedAtBlockNumber, AuRaParameters.Validator validatorPrototype)
        {
            if (validatorPrototype != _currentValidatorPrototype)
            {
                ...
                if (CanChangeValidatorImmediately(validatorPrototype))
                {
